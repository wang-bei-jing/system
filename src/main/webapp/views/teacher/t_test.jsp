...
<script type="text/javascript">
    ...
    function build_time_table(studentCommentList){
        ...
        for(var i=0;i<studentCommentList.length;i++){
            ...
            var resultTd = $("<td class='peace'></td>").attr("title",studentCommentList[i].teacherComment.tcContent).append(studentCommentList[i].teacherComment.tcContent);
            var teacherTime = timeStamp2String (studentCommentList[i].teacherComment.tcTime);
            var teacherTimeTd = $("<td></td>").append(teacherTime);
            var editBtn = $("<button></button>").addClass("btn btn-primary btn-sm edit_btn")
                .append($("<span></span>").addClass("glyphicon glyphicon-pencil"))
                .append("回复");
            //为同意按钮添加一个自定义的属性来表示当前同意的课题选择id
            editBtn.attr("edit-id",studentCommentList[i].tiId);
            var delBtn =  $("<button></button>").addClass("btn btn-danger btn-sm delete_btn")
                .append($("<span></span>").addClass("glyphicon glyphicon-trash"))
                .append("修改");
            //为删除按钮添加一个自定义的属性来表示当前拒绝的课题选择id
            delBtn.attr("del-id",studentCommentList[i].tiId);
            var btnTd = $("<td></td>");
