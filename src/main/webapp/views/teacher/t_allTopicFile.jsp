...
<script type="text/javascript">
    ...
    function to_page(pn){
        $.ajax({
            url:"${pageContext.request.contextPath}/file/findAll/",
    ...
    $(document).on("click",".delete_btn",function(){
        var teacher = $(this).parents("tr").find("td:eq(4)").text();
        var fileName = $(this).parents("tr").find("td:eq(2)").text();
        var dId = $(this).attr("del-id");
        if(confirm("确认下载"+teacher+"的"+fileName+"吗？")){
    ...
    $("#download_all_btn").click(function(){
        ...
        $.each($(".check_item:checked"),function(){
            ...
            var id = $(this).parents("tr").find("td:eq(6)").find("button:eq(0)").attr("del-id");
            ids.push(id);
        });
